...
%% }
-spec weave(SrcDir, MfaSpec, Opts) -> [comp_ret()] when
    SrcDir :: string(),
...
%%           Warnings are included if present.
%% }
-spec weave_file(SrcFile, MfaSpec, Opts) -> comp_ret() when
    SrcFile :: string(),
    MfaSpec :: analyzer:mfa_spec(),
    Opts :: options().
...
-spec parse_transform(Ast, Opts) -> Ast0 :: [erl_parse:abstract_form()] when
...
write_file(File, MfaSpec, Opts) ->
    ...
    CompileOpts = [
        {i, opts:include_opt(Opts)},
        {outdir, opts:out_dir_opt(Opts)},
        return,
        {?OPT_MFA_SPEC, MfaSpec},
        {?OPT_FILTER, filter_opt(Opts)},
...
trans_expr(
    Expr = {call, _, {remote, _, {atom, _, erlang}, {atom, _, How}}, _}, MfaSpec, FilterSpec, Id
) when
    How =:= spawn; How =:= spawn_link
->
...
trans_expr(
    Expr = {call, _, {remote, _, {atom, _, proc_lib}, {atom, _, How}}, _}, MfaSpec, FilterSpec, Id
) when
    How =:= spawn; How =:= spawn_link
->
...
trans_expr(Expr = {call, _, {atom, _, How}, Args}, MfaSpec, FilterSpec, Id) when
    length(Args) =:= 3, How =:= spawn; How =:= spawn_link
->
...
%% handled via tracing.
weave_remote_spawn(
    {call, Line, {remote, _, ModSpawn = {atom, _, erlang}, FunSpawn = {atom, _, How}}, SpawnArgs},
    MfaSpec,
    FilterSpec,
    Id
) when
...
->
    ...
    % transformed.
    {Id0, SpawnArgs0} = trans_args(
        [ModMatch, FunMatch, ArgsMatch],
        MfaSpec,
        FilterSpec,
        Id + 1
    ),
    ...
    Case = abs_case(Line, MfaSpecCall, [
        abs_case_clause(Line, [abs_atom(Line, undefined)], [], [
            abs_remote_call(Line, ModSpawn, FunSpawn, SpawnArgsVars)
        ]),
        abs_case_clause(Line, [abs_tuple(Line, [abs_atom(Line, ok), MonFunVar])], [], [
            PidParentMatch, SpawnCall
        ])
    ]),
...
%% FASE 2021.
weave_remote_proc_lib_spawn(
    {call, Line, {remote, _, ModSpawn = {atom, _, proc_lib}, FunSpawn = {atom, _, How}}, SpawnArgs},
    MfaSpec,
    FilterSpec,
    Id
) when
...
abs_list(_, [Expr | Exprs]) ->
    % Second element of expression is ALWAYS line number.
    Line = element(2, Expr),
...
create_dispatch(Line, Event) ->
    abs_remote_call(
        Line,
        abs_atom(Line, ?TRACE_MOD),
        abs_atom(Line, ?TRACE_FUN),
        [Event]
    ).
